...
public class Testgarib
{
	public int solution(int[] A, int Y)
	{
		LinkedHashMap<Integer, Integer> uniqueNails = new LinkedHashMap<Integer, Integer>();
		...
		{
			if (uniqueNails.containsKey(A[i]))
			{
				uniqueNails.put(A[i], uniqueNails.get(A[i]) + 1);
			}
			else
			{
				uniqueNails.put(A[i], 1);
			}
		}
		// covert uniqueNails map to array
		// uniqueNails [1=2, 3=4, 4=1, 5=4]
		// nailCount [2, 4, 1, 4]
		for(Entry<Integer, Integer> ent : uniqueNails.entrySet())
			nailCount.add(ent.getValue());
		
		// calculate number nails of higher lengths
		// nailCount [2, 4, 1, 4]
		// sumNailCount [8, 5, 4, 0]
		...
		int max = 0;
		// calculate max of number of nails with same length by comparing with Y
		for (int i = 0; i < nailCount.size(); i++)
		{
	...
	public static void main(String[] args)
	{
		...
		Testgarib solution = new Testgarib();
		System.out.println(solution.solution(A, 6));
	}
}
